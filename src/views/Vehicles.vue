<script setup>
	...
	import VehicleSubPage from "./VehicleSubPage.vue";
	...
	const { data: vendorSearchResults, isFetching: isVendorSearchFetching } =
		useQuery(["vehicleSearch", debouncedSearchText], ({ queryKey }) => {
			if (queryKey[1] === "") {
				return null;
			} else {
	...
	onMounted(() => {
		router.push({ hash: global.pages.inventory.activeTab });
	});
	...
	watch(
		() => global.pages.inventory.activeTab,
		(val) => {
	...
	<div class="vendors flex w-full">
		...
		>
			...
			<div
				...
				class="absolute bottom-[-48px] left-0 flex h-[48px] w-[276px] cursor-pointer flex-row items-center justify-between bg-background_light/50 bg-black px-4 shadow-[0_-3px_11px_-5px_rgba(0,0,0,0.25)] duration-[500ms] dark:bg-dark_border/50"
...
<style lang="scss" scoped>
	...
	.__labeled-data {
		@apply border-b-[2px] border-background_light p-[6px] pl-[12px] dark:border-dark_border;
		span {
