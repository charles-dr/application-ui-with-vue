...
const { data: dealer, isLoading } = useQuery(["dealer", routeParamId], ({ queryKey }) =>
  axios.get(`/dealers/${queryKey[1]}`).then((res) => res.data)
);
...
<template>
  <div
    id="details"
    class="Section __dealer-card Details mt-0 grid grid-cols-12 rounded-round bg-foreground_light p-6 dark:bg-foreground_dark"
  >
    <!-- left side -->
    <div class="__form col-span-12 flex flex-col justify-between md:col-span-12">
      <div class="__title col-span-6">
        <h3 class="mb-2 translate-x-2 font-bold uppercase opacity-[0.44]">Dealer</h3>
        <CustomInput
          type="header"
          placeholder="Name"
          :value="form.name"
  ...
  <DealerContacts id="dealer-contacnts" class="Section" />
</template>
