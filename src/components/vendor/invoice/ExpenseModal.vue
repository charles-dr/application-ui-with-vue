...
const expenseTypeOptions = computed(() => {
  ...
  expenseTypeComputed = expenseTypeComputed.filter(
    (el, index, array) => index === array.findIndex((arr) => arr.value === el.value)
  );
...
// Get Array Item from n-select options value
  ...
  <n-form :model="form" ref="formRef" @submit.prevent="onSubmitForm" :rules="rules">
    ...
    <div class="sm:flex sm:gap-x-5">
      ...
      <n-form-item path="adjusted_amount" label="Adjusted Amount">
        <CurrencyInput v-model="form.adjusted_amount" :loading="isLoading" />
      </n-form-item>
    </div>
