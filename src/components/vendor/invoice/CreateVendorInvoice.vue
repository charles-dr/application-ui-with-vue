...
watch(
  ...
  (newValue) => {
    if (newValue) {
      ...
      form.value.due_date = current_vendor.value
        ? dayjs().add(current_vendor.value?.payment_terms.days, "day").valueOf()
        : dayjs().valueOf();
  ...
  >
    ...
    >
      ...
      <div class="sticky bottom-2 flex gap-x-5">
        <n-button type="primary" :loading="isLoading" @click.prevent="submitInvoice">
          CREATE INVOICE
        </n-button>
      </div>
    </n-form>
