...
import { getDealIdByVin, getDocuments, getDocumentTypeOptions } from "@/hooks/document";
import {
...
const { data: dealId, isLoading: isDealsLoading } = getDealIdByVin(routeParamId);
const { data: documents, isLoading: isDocsLoading, refetch } = getDocuments(dealId);
...
const initDynamSoft = () => {
  ...
  Dynamsoft.DWT.ProductKey = import.meta.env.DWT_LICENSE_Key;
  Dynamsoft.DWT.organizationID = import.meta.env.DWT_LICENSE_ID;
...
const Dynamsoft_OnReady = () => {
  ...
  if (DWObject.value) {
    if (DWObject.value.ErrorCode == 0) {
      ...
      DWObject.value.RegisterEvent("CloseImageEditorUI", Dynamsoft_CloseImageEditorUI);
      DWObject.value.RegisterEvent("OnBitmapChanged", Dynamsoft_OnBitmapChanged);
      ...
      DWObject.value.RegisterEvent("OnPostAllTransfers", Dynamsoft_OnPostAllTransfers);
      DWObject.value.RegisterEvent("OnGetFilePath", Dynamsoft_OnGetFilePath);
      DWObject.value.Viewer.on("pageAreaSelected", Dynamsoft_OnImageAreaSelected);
      DWObject.value.Viewer.on("pageAreaUnselected", Dynamsoft_OnImageAreaDeselected);
    }
  }
};
/**
...
const openImage = () => {
   ...
   */
  DWObject.value.Addon.PDF.SetConvertMode(Dynamsoft.DWT.EnumDWT_ConvertMode.CM_RENDERALL);
...
const btnRemoveCurrentImage_onclick = () => {
  ...
  confirmDialog.error({
    ...
    onPositiveClick: () => {
      ...
      if (DWObject.value.HowManyImagesInBuffer == 0) DWObject.value.RemoveImage(0);
...
const btnZoomIn_onclick = () => {
  ...
  var zoomInStep = 5;
  DWObject.value.Viewer.zoom = (DWObject.value.Viewer.zoom * 100 + zoomInStep) / 100.0;
  updateZoomInfo();
};
...
const btnZoomOut_onclick = () => {
  ...
  var zoomOutStep = 5;
  DWObject.value.Viewer.zoom = (DWObject.value.Viewer.zoom * 100 - zoomOutStep) / 100.0;
  updateZoomInfo();
};
...
const uploadToServer = () => {
  ...
  var OnFailure = (errorCode, errorString, httpResponse) => {
    if (errorCode != 0 && errorCode != -2003) saveFileFormMessage.error(errorString);
    else {
      saveFileFormMessage.success("Successfully uploaded");
      initFormValue();
      refetch.value();
...
<template>
  <Card class="Documents w-full mt-[-24px]">
    <div
      class="Documents__actions w-full flex-col !md:flex justify-center !md:justify-start"
    >
      <n-button
        class="Douments__action Documents__action--scan w-full md:w-[220px]"
        @click="handleScan"
        >Scan Document</n-button
      >
      <span class="w-full mt-[12px] md:ml-[10px] md:w-auto md:mt-[0px]"
  ...
  >
    ...
    <div id="DWTcontainer" class="grid grid-cols-3 gap-4">
      <div class="col-span-3 md:col-span-2">
        <div id="divEdit">
          <n-button :bordered="false" @click="btnRemoveCurrentImage_onclick" class="ml-1">
            <img
              class="menuIcon"
          ...
          <span class="page-detail">{{ currentPageNum }} / {{ totalPageNum }}</span>
          <n-button :bordered="false" @click="btnZoomOut_onclick" class="ml-1 mr-3">
            <img
              class="menuIcon"
          ...
          <n-button :bordered="false" @click="btnZoomIn_onclick" class="ml-1 mr-3">
            <img
              class="menuIcon"
          ...
          <n-button :bordered="false" @click="btnRotateLeft_onclick" class="ml-1 mr-3">
            <img
              class="menuIcon"
          ...
          <n-button :bordered="false" @click="btnCrop_onclick" class="ml-1 mr-3">
            <img
              class="menuIcon"
              src="/images/Crop.png"
              title="Crop"
          ...
          <n-button :bordered="false" @click="btnSelect_onclick" class="ml-2 mr-3">
            <img
              class="menuIcon"
              :src="selectOnClick ? '/images/Select_selected.png' : '/images/Select.png'"
              title="Select"
              alt="Select"
              id="btnSelect"
            />
          </n-button>
          <n-button :bordered="false" @click="btnHand_onclick" class="ml-0 mr-3">
            <img
              class="menuIcon"
              :src="handOnClick ? '/images/Hand_selected.png' : '/images/Hand.png'"
              title="Hand"
              alt="Erase"
              id="btnHand_selected"
            />
      ...
      <div class="col-span-3 md:col-span-1">
        <n-tabs default-value="save-documents" justify-content="space-evenly" type="line">
          <n-tab-pane
            name="save-documents"
            tab="Save Document(s)"
          ...
          >
            <div class="mb-[20px]">
              <select
                v-if="!bWASM"
                class="col-span-3 md:col-span-2 w-full dark:bg-[#333333]"
                id="sources"
              ></select>
              <div class="mt-[10px] mb-[10px] flex justify-between gap-[10px]">
                <n-button class="grow" v-if="!bWASM" @click="acquireImage()"
                  >Scan Image</n-button
                >
                <n-button class="grow" @click="openImage()">Import Local Image</n-button>
              </div>
            </div>
            <n-form
              ref="saveFileFormRef"
            ...
            >
              <n-form-item label="Title" path="title" class="col-span-3 md:col-span-2">
                <n-input v-model:value="saveFileFormValue.title" placeholder="Title" />
              </n-form-item>
              <n-form-item
                label="File Type"
                path="file_type"
              ...
              <n-form-item label="Type" path="doc_type" class="col-span-3 md:col-span-3">
                <n-select
                  class="w-full"
          ...
          <n-tab-pane name="scan-options" tab="Scan Options" display-directive="show">
            <div class="grid grid-cols-2 gap-2">
              <n-checkbox
                v-model:checked="showScannerUI"
                class="col-span-3 md:col-span-1"
              >
                Show Scanner UI
              </n-checkbox>
              <n-checkbox v-model:checked="useAdf" class="col-span-3 md:col-span-1">
                Use ADF
              </n-checkbox>
              <n-checkbox
              ...
              <n-checkbox v-model:checked="twoSidedScan" class="col-span-3 md:col-span-1">
                2-sided Scan
              </n-checkbox>

              <div class="col-span-3 md:col-span-2 mt-2 flex items-center justify-start">
                <span class="mr-2">Pixel Type:</span>
                <n-radio-group v-model:value="pixelType" name="pixelTypeGroup" class="">
                  <n-space>
                    <n-radio
  ...
  <n-modal
    class="no-padding max-w-[1220px]"
    v-model:show="viewDocumentModal"
    preset="card"
    title="View Document"
  >
    <!-- Content -->
    ...
    >
      <n-button class="mt-2 self-auto" @click.prevent="download(viewDocumentModal.url)">
        <n-icon
          ><svg
