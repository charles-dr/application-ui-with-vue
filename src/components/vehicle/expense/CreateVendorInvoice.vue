...
import { ref, watch, computed, h, toRaw } from "vue";
import dayjs from "dayjs";
import { getInvoiceStatus } from "@/hooks/common_query";
import { themeOverrides } from "@/components/vehicle/expense/expense.helper";
...
const { data: vendorSearchResults, isFetching: isVendorSearchFetching } =
  useQuery(["vendorSearch", debounceSearchVendor], ({ queryKey }) => {
...
const { mutateAsync: createExpense, isLoading } = useMutation(
  ...
  {
    onSuccess() {
      queryClient.invalidateQueries(["vendorInvoices"]);
    },
  }
);
...
const onSaveExpense = (expense) => {
  ...
  if (editExpenseIndex.value !== null) {
    ...
    form.value.expenses.push({
      ...
      deal: { id: props.deal.id, vin: props.deal.vin },
    });
  }
};
</script>
<template>
  <n-button v-bind="$attrs" @click="showDrawer = true">
    <n-icon class="mr-1">
      <svg
    ...
    >
      ...
      >
        <header class="flex content-center justify-between">
          ...
          <section class="flex w-1/2 flex-col items-end gap-y-3">
            <div>
              <n-select
