...
import { themeOverrides } from "./expense.helper";
...
const debounceSearchVendor = useDebounce(searchVendor, 500);
const {
  data: vendors,
...
const { data: vendorSearchResults, isFetching: isVendorSearchFetching } = useQuery(
  ["vendorSearch", debounceSearchVendor],
  ({ queryKey }) => {
...
const vendorOptions = computed(() => {
  let computedVendors = vendorSearchResults.value
    ? vendorSearchResults.value?.map((vendor) => ({
        label: vendor.name,
        value: vendor.id,
      }))
    : vendors.value?.pages.reduce(
        (prev, current) =>
          prev.concat(
  ...
  if (props.initialData.vendor) {
    computedVendors = computedVendors.concat([
      {
        value: props.initialData.vendor[0].id,
        label: props.initialData.vendor[0].name,
      },
    ]);
  }
  return computedVendors;
});
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      form.value = pick(props.initialData, [
        "vendor_id",
        "status",
        "invoice_number",
        "id",
      ...
      form.value.due_date = dayjs(props.initialData.due_date).valueOf();
      form.value.invoice_date = props.initialData.invoice_date
        ? dayjs(props.initialData.invoice_date).valueOf()
        : null;
      ...
      form.value.expenses = props.initialData.vendor_expenses.map((expense) => ({
        ...pick(expense, [
          "deal",
          "deal_id",
          "description",
...
<template>
  <n-form ref="formRef" :model="form" :rules="rules">
    <n-config-provider :theme-overrides="themeOverrides" inline-theme-disabled>
      <header class="flex content-center justify-between">
        <section class="space-y-4">
          ...
          <n-form-item label="Vendor" path="vendor_id">
            <n-select
              class="other-select"
              ...
              :loading="isVendorSearchFetching || isVendorsLoading"
              @search="(query) => (searchVendor = query)"
            />
          </n-form-item>
        </section>
    ...
    <div
      ...
      v-if="(!disabled && form.amount_paid === 0) || form.amount_due > 0"
    >
      <n-button
        size="medium"
        type="primary"
