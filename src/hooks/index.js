...
export function fetchById(path, paramId) {
  return useQuery([queryName(path, "ById"), paramId], () =>
    axios.get(`${path}/${paramId.value}`).then((res) => {
      return res.data;
    })
  );
}
...
export const useReactiveRouteParams = (key) => {
  const route = useRoute();
  const routeParamId = ref(route.params[key]);
  watch(
    () => route.params,
    (toParam) => {
      if (toParam[key]) routeParamId.value = toParam[key];
    },
    { immediate: true }
  );
  return { routeParamId };
